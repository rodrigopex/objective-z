...
// `OZNumber` (`common::oznumber_src`) for the boxed integer elements, and
...
// `companion::render_array_support`. Only `OZObject`, `OZNumber`, and
...
mod common;
use common::{compile_and_run, ozarray_src, ozobject_src as PREAMBLE, oznumber_src};
...
",
        PREAMBLE(), oznumber_src(), ozarray_src()
    );
...
/// comma list, with each element rendered by OZNumber's override.
#[test]
...
int main(void) {
	...
	[nums enumerateObjectsUsingBlock:^(id obj, size_t idx, BOOL *stop) {
		g_calls = g_calls + 1;
		g_sum = g_sum + [((OZNumber *)obj) int32Value];
	}];
	printf(\"calls=%d\\n\", g_calls);
	printf(\"sum=%d\\n\", g_sum);
