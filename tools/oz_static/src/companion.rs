...
// (it only casts pointers and calls functions, never dereferences).
...
pub fn render(
    ...
    for name in &struct_order {
        match struct_text.get(name.as_str()) {
            // Root: full definition -- retain/release/dispatch need it.
            Some(text) => {
                h.push_str(text);
                h.push('\n');
            }
            // Everyone else: forward declaration only. The full struct
            // lives in-place at this class's own @interface.
            None => {
    ...
    // Only the root's alloc/free is defined here -- every other class's
    // is defined in-place, next to its own struct.
    if let Some(root) = &root {
