...
pub fn render(
    ...
    // `id` (the untyped "any object pointer" -- see `collect::render_type`,
    // which resolves an `id` parameter/return type to `void *` in method
    // signatures) still appears verbatim, untranslated, in every context
    // this spike copies through as raw source text: ivar declarations,
    // the inner parameter list of a `(^)`-to-`(*)`-converted block type,
    // and a plain top-level C function's own signature. Defining it once
    // here -- included by both the primary source and this companion --
    // makes all of those compile too, instead of only method signatures.
