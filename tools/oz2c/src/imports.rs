...
/// to a comment, and a `@compatibility_alias` is not one of the
/// constructs `preproc::TOP_LEVEL_OBJC` names -- so `#ifdef __clang__`
/// around one is still copied through rather than resolved, and left
/// wrapped the alias would pass through as invalid raw ObjC text.
///
/// **Narrower than it was.** This comment used to say emit "doesn't
/// recurse into `#ifdef`/`#endif` conditionals to find one nested
/// inside", flatly. Since #573 it does, for the five class-declaring
/// constructs: `preproc::Liveness` decides the arm and
/// `effective_top_level` splices it in. This line-based unwrap survives
/// because the alias is not in that set and because it runs *before* the
/// merge -- it deletes lines, which shifts every line number after them,
/// so #305's source map is built on its output
/// (`unwrap_clang_guard_tracked`'s second return value is what undoes
/// the shift). Folding it into `preproc` would move generated bytes for
/// no behavioural gain; tracked separately.
///
/// Shared with `tests/common/mod.rs`'s hand-assembled fixtures, which
/// hit the exact same headers.
