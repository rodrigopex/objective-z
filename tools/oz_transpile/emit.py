    ...
    if kind == "ReturnStmt":
        ...
        if len(inner) >= 2:
            ...
            if inner[1].get("kind") == "CompoundStmt":
                _emit_compound_stmt(inner[1], out, cls, module, root_class, indent, inline=True)
            else:
                out.write("{\n")
...
def _emit_if_stmt(node: dict, out: StringIO, cls: OZClass, module: OZModule,
    ...
    if then_body:
        if then_body.get("kind") == "CompoundStmt":
            _emit_compound_stmt(then_body, out, cls, module, root_class, indent, inline=True)
        else:
            out.write("{\n")
    ...
    if else_body:
        if else_body.get("kind") == "IfStmt":
            ...
            out.write(f"{tabs}else ")
            _emit_compound_stmt(else_body, out, cls, module, root_class, indent, inline=True)
        else:
            out.write(f"{tabs}else {{\n")
...
def _emit_for_stmt(node: dict, out: StringIO, cls: OZClass, module: OZModule,
    ...
    if body and body.get("kind") == "CompoundStmt":
        _emit_compound_stmt(body, out, cls, module, root_class, indent, inline=True)
    elif body:
        out.write("{\n")
...
def _emit_functions(module: OZModule, outdir: str, root_class: str) -> str:
    ...
    for func in module.functions:
        ...
        if func.body_ast:
            _emit_compound_stmt(func.body_ast, out, dummy_cls, module,
                                root_class, indent=0)
        else:
            out.write("{\n}\n")
        out.write("\n")
