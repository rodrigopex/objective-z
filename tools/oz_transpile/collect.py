...
def _is_macro_expanded(node: dict) -> bool:
    """Check if a node was generated by macro expansion (spellingLoc in scratch space)."""
    loc = node.get("loc", {})
    spelling = loc.get("spellingLoc", {})
    file_path = spelling.get("file", "")
    return file_path == "<scratch space>"
...
def _walk(node: dict, module: OZModule, impl_name: str | None = None,
    ...
    if kind == "ObjCInterfaceDecl":
        ...
        if (_is_from_main_file(node) and node.get("storageClass") == "static"
                and not _is_macro_expanded(node)):
...
def _is_user_include(line: str, source_dir: Path | None = None) -> bool:
    """Check if an include should be propagated to generated code.

    Skips #import directives and OZ stub includes.  For quoted includes,
    checks if the referenced file contains ObjC syntax (@interface etc.)
    and skips those since the transpiler generates replacement headers.
    """
    stripped = line.strip()
    if stripped.startswith("#import"):
        return False
    for prefix in _OZ_INCLUDE_PREFIXES:
        if prefix in line:
            return False
    if '"' in stripped and source_dir:
        import re
        m = re.search(r'"([^"]+)"', stripped)
        if m:
            candidates = [
                source_dir / m.group(1),
                source_dir.parent / "include" / m.group(1),
            ]
...
def _distribute_verbatim_to_classes(module: OZModule) -> None:
    """Move verbatim lines and user includes to classes with implementations.
