/* Auto-generated by oz_transpile -- do not edit */
#include "oz_dispatch.h"
#include "OZObject_ozh.h"
#include "Logger_ozh.h"
#include "Timer_ozh.h"

const char *const oz_class_names[OZ_CLASS_COUNT] = {
	[OZ_CLASS_OZObject] = "OZObject",
	[OZ_CLASS_Logger] = "Logger",
	[OZ_CLASS_Timer] = "Timer",
};

const uint8_t oz_superclass_id[OZ_CLASS_COUNT] = {
	[OZ_CLASS_OZObject] = OZ_CLASS_COUNT,
	[OZ_CLASS_Logger] = OZ_CLASS_OZObject,
	[OZ_CLASS_Timer] = OZ_CLASS_OZObject,
};

const OZ_fn_dealloc OZ_PROTOCOL_RESOLVE_dealloc[OZ_CLASS_COUNT] = {
	[OZ_CLASS_OZObject] = (OZ_fn_dealloc)OZObject_dealloc,
	[OZ_CLASS_Logger] = (OZ_fn_dealloc)Logger_dealloc,
	[OZ_CLASS_Timer] = (OZ_fn_dealloc)Timer_dealloc,
};

const OZ_fn_init OZ_PROTOCOL_RESOLVE_init[OZ_CLASS_COUNT] = {
	[OZ_CLASS_OZObject] = (OZ_fn_init)OZObject_init,
	[OZ_CLASS_Logger] = (OZ_fn_init)OZObject_init,
	[OZ_CLASS_Timer] = (OZ_fn_init)OZObject_init,
};

void OZObject_dispatch_free(struct OZObject *obj)
{
	switch (obj->oz_class_id) {
	case OZ_CLASS_OZObject: OZObject_free((struct OZObject *)obj); break;
	case OZ_CLASS_Logger: Logger_free((struct Logger *)obj); break;
	case OZ_CLASS_Timer: Timer_free((struct Timer *)obj); break;
	default: break;
	}
}
