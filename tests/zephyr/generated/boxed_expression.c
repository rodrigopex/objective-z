...
void BoxedTest_run(struct BoxedTest *self)
{
	int val = 7;
	/* _fromVar = @(val); */
	(oz_static_release((struct OZObject *)(self->_fromVar)), self->_fromVar = OZNumber_numberWithInt32__cls((val)));
	/* _fromExpr = @(val + 3); */
	(oz_static_release((struct OZObject *)(self->_fromExpr)), self->_fromExpr = OZNumber_numberWithInt32__cls((val + 3)));
	/* _fromCall = @(triple(val)); */
	(oz_static_release((struct OZObject *)(self->_fromCall)), self->_fromCall = OZNumber_numberWithInt32__cls((triple(val))));
	float f = 2.5f;
	/* _fromFloat = @(f); */
	(oz_static_release((struct OZObject *)(self->_fromFloat)), self->_fromFloat = OZNumber_numberWithFloat__cls((f)));
	unsigned int u = 1000;
	/* _fromUint = @(u); */
	(oz_static_release((struct OZObject *)(self->_fromUint)), self->_fromUint = OZNumber_numberWithInt32__cls((u)));
}
