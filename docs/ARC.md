...
| — | A receiver's class must resolve the same way for ownership as for dispatch | `IMPLEMENTED`, and **enforced rather than tracked** since #483 | The rule was aspirational while it was written down: it required two enumerations to agree and nothing checked that they did. #481 taught `arc::collect_declared_types` the `method_parameter` kind and #502 taught it the for-in binding, each closing one instance — and the class stayed open, because a shape that is **not a node kind** cannot be enumerated (an ivar and a file-scope variable are declared *outside* the method scope `arc::declared_class_of` walks, and a cast receiver is not an `identifier` at all). #483 refuses the disagreement instead: `emit::reject_static_dispatch_contradiction` fires when the emitter has resolved a receiver and `arc::polled_dispatch_ownership` answers `Ambiguous`, which can only mean the poll ran over classes the send can never reach. A contradiction, not an ambiguity, so a located build error rather than the borrowed reading that leaks. `static_dispatch_contradiction.rs` pins the three shapes that were live (ivar, file-scope, cast) against four controls that must stay accepted; `parameter_receiver_class.rs` and `forin_receiver_class.rs` keep the two resolved forms resolved |
