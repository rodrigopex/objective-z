...
**This recurred, identically, under #570 -- nine SDK sources, each
`@implementation` twice -- and the paragraph above is why it was not
prevented.** It says to hold the include root constant, which reads as
something the invocation controls. It is not: `main.rs` computes its default
`-I include/oz_sdk` and `--impl-dir src` from `env!("CARGO_MANIFEST_DIR")`,
**baked in at compile time**, and always prepends them. So passing `-I` and
`--impl-dir` explicitly -- which #570's sweep did -- does not help, and cannot:
a binary built in checkout A carries A's `src` as a search path wherever it
runs. The entry `src/OZObject.m` then canonicalises under the *calling*
checkout while its sibling implementation is found under the *building* one,
`resolve_entry_files`' `seen` set sees two distinct paths to one file, and it
splices twice.

So the rule is stronger than "hold the invocation constant": **build both
binaries in the same checkout.** Swap the sources under one tree, build, save
the binary aside, restore, build again. A worktree per binary is exactly the
shape that fails. The corrected sweep gave 145 of 145 byte-identical, and the
12 inputs both binaries refuse were compared on their stderr rather than
...
- **A preprocessor conditional around Objective-C is oz2c's decision, settled
  ...
  arms of every conditional used to reach every pass. `preproc::Liveness`
  decides which arm is part of the program, once, and the verdict is carried on
  ...
  **It cannot be deferred to the C compiler**, and that is the whole reason the
  rule exists rather than being a preference. A class becomes a struct, a row
  in the shared dispatch table, and a slab sized from its allocation sites:
  whole-program artifacts, in files the conditional does not span. There is no
  way to hand GCC half a dispatch table. So a resolved conditional **does not
  survive into the generated C** -- the live arm is lowered and the directives
  are gone.

  Three boundaries, each of which a well-meaning change will want to move:

  - **A conditional carrying no Objective-C is untouched.** Its text is copied
    through and the C compiler decides, which is how `#ifndef OZ_Q31_HELPERS`
    and every other SDK guard works. The scope is
    `preproc::TOP_LEVEL_OBJC` -- the five class-declaring constructs -- and
    widening it moves generated bytes for every guard in the tree.
  - **Absent from the buffer means undefined, except in two namespaces.** That
    is C's own rule and it is the only thing that makes `#ifdef
    MT96_NEVER_DEFINED` decidable. It is *unsound* for names reserved to the
    implementation (`__GNUC__`, defined by the compiler; `is_reserved_name`
    implements C17 7.1.3 as a shape, not a list) and for `CONFIG_` (Zephyr's
    Kconfig, arriving through `-include autoconf.h`). Both are
    `Truth::Unknown`. Treating them as absent would have silently selected the
    `#else` arm of every `#ifdef CONFIG_FOO` a real application writes, which
    is the one outcome worse than a refusal.
  - **An unevaluatable condition guarding Objective-C is a hard located
    error**, per the no-degrade rule. The alternative is not a warning -- it is
    #573's broken C, a raw `@interface` in the generated header with no struct,
    no method and no dispatch row behind it.

  A dead arm also stops *counting*: `pools.rs` sized slabs over the whole tree,
  so three `[Foo alloc]`s inside `#if 0` took a class's slab from 1 slot to 4
  -- static storage on a heapless target, reserved for sends the C compiler
  never sees. That half had no reporter and no symptom: the generated C
  compiles and runs correctly either way.

  Knowing an arm is dead suppresses *diagnostics and counting* about it, never
  the author's text. A dead arm is still copied through.

  Diagnostics are filtered once, where they leave a pass
  (`Liveness::retain_live`), rather than in each check. There are 306 tree
  walks in this crate; one of them firing on `#if 0` text was the whole of
  #570, and the ones nobody has written yet would each have had to know to
  ask.
