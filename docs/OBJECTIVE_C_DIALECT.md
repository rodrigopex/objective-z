...
| `block.ozfn-typecheck` | the *contents* of `OZFN`/`OZM` | `GAP` | `OZFN` expands to `0` for Clang, so the AST never contains the block and nothing checks the source inside it. A signature mismatch surfaces as GCC on generated C, pointing into a generated file. #548 closed the missing-brace half; the typecheck half is #603. | #603; oz2c-challenges R2 |
