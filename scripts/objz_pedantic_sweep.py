...
make it read *clean* rather than fail, which is worse:
  ...
  1. **`-Wpedantic` cannot go in `EXTRA_CFLAGS`.** That applies to
     ...
     files under `oz2c_generated/` are recompiled afterwards, from
...
# oz2c_generated/, message) and valued (sites, reason). `*` as the
...
DIAG_RE = re.compile(
    r"^(?P<file>\S*oz2c_generated\S*):(?P<line>\d+):(?P<col>\d+): "
...
def build(sample, board, build_dir):
    ...
    `oz2c_transpile_gen` (`cmake/oz2c.cmake`) depends on oz2c's
...
def generated_tus(build_dir):
    with open(os.path.join(build_dir, "compile_commands.json")) as f:
        return [e for e in json.load(f) if "oz2c_generated" in e["file"]]
...
def recompile(entry, build_dir, wrapdir):
    ...
    sites, paths relative to oz2c_generated/.
    """
    # Named after the TU's path within oz2c_generated/, not its
    # basename. Two generated TUs can share a basename -- a class's own
    # `.c` beside `Foundation/`'s -- and with the recompiles running
    # concurrently two threads writing one wrapper is a race that would
    # show up as a diagnostic attributed to the wrong file, or none at
    # all. Serially it was merely harmless reuse.
    rel = entry["file"].split("oz2c_generated/", 1)[-1]
    wrap = os.path.join(wrapdir, rel.replace(os.sep, "_"))
    ...
    for line in p.stderr.splitlines():
        ...
        if m:
            rel = m.group("file").split("oz2c_generated/", 1)[1]
...
def sweep_sample(sample, board, keep):
    ...
    # Concurrent, because this is where the sweep's time is once the
    # builds stop being the bottleneck. Each entry is an independent
    # compiler invocation -- ~24 per sample, ~300 over the whole sweep --
    # and on CI that pass is essentially the whole runtime: the sample
    # builds there share one job and Zephyr's objects barely differ
    # between samples, so ccache makes all but the first nearly free
    # (#394).
    #
    # Threads rather than processes: every worker spends its life in
    # `subprocess.run`, which releases the GIL, so the parallelism is
    # real and nothing needs pickling.
    #
    # Order does not matter -- results are unioned into a set and counted
    # afterwards -- so the report is identical whatever order they finish
    # in. That is worth stating because it is what makes this safe to
    # parallelise at all.
    entries = generated_tus(build_dir)
    sites = set()
    workers = max(1, min(len(entries), os.cpu_count() or 2))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        for found in pool.map(
                lambda e: recompile(e, build_dir, wrapdir), entries):
            sites |= found
    if not keep:
        shutil.rmtree(build_dir, ignore_errors=True)
