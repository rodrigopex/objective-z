...
| `just test-behavior`      | Behavior corpus, 75 cases; `--compiler`/`--opt`/`--sanitize`/`--check-leaks` |
...
- all 71 behaviour and 40 adapted cases *as the corpus stood then* transpile **and
  ...
  LeakSanitizer (the behaviour corpus is 75 cases now);
...
- **`tests/behavior/`** — 75 compiled behavior tests across 17 categories (Unity framework, host-side)
