...
function(_objz_collect_compile_db source object)
    ...
    if(NOT _deferred)
        ...
        # Cache paths now — they may not resolve in the deferred context.
        get_filename_component(_module_dir "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/.." REALPATH)
        set_property(GLOBAL PROPERTY _OBJZ_MODULE_DIR ${_module_dir})
        get_filename_component(_proj_root "${_module_dir}/.." REALPATH)
        set_property(GLOBAL PROPERTY _OBJZ_PROJECT_ROOT ${_proj_root})
...
function(_objz_write_compile_db)
    ...
    get_property(_module_dir GLOBAL PROPERTY _OBJZ_MODULE_DIR)
    get_property(_project_root GLOBAL PROPERTY _OBJZ_PROJECT_ROOT)
    ...
    file(GLOB_RECURSE _all_m_files
        "${_project_root}/samples/*.m"
        "${_project_root}/tests/*.m"
        "${_project_root}/benchmarks/*.m"
        "${_module_dir}/src/*.m")
    ...
    add_custom_target(objz_compile_db ALL
        COMMAND ${Python3_EXECUTABLE}
                ${_module_dir}/scripts/objz_merge_compile_db.py
    ...
    # Generate .clangd at project root with the correct target triple.
    # clangd derives a wrong target from GCC cross-compiler names
    # (e.g. arm-zephyr-eabi-gcc -> armv4t). Override with the Clang triple
    # computed from Zephyr's CPU config.
    _objz_get_clang_target_triple(_clangd_triple)
    file(WRITE "${_project_root}/.clangd"
"# Auto-generated by ObjcClang.cmake — do not edit manually.\n\
# Regenerated on every CMake configure (just build / just rebuild).\n\
\n\
CompileFlags:\n\
  CompilationDatabase: build\n\
  Add:\n\
    - -I${_module_dir}/include/oz_transpile\n\
  Remove:\n\
    - --target=*\n\
    - -fobjc-runtime=*\n\
    - -specs=*\n\
...
Diagnostics:\n\
  ...
  Suppress:\n\
    - drv_unknown_attribute\n\
    - unknown_attribute\n")
    message(STATUS "Objective-Z: generated ${_project_root}/.clangd (target=${_clangd_triple})")
...
function(objz_compile_objc_sources target)
    ...
    foreach(_src ${ARGN})
        ...
        _objz_collect_compile_db(${_abs} ${_obj} ${_clang_flags} ${_extra_incs})
