...
#
# objz_gen_pools.py - Generate OZ_DEFINE_POOL() calls from Clang AST analysis.
#
...
#
# Usage:
#   python3 objz_gen_pools.py --pointer-size=4 --output=pools.c f1.ast.json ...
...
def generate(pools, output_path):
    with open(output_path, "w") as f:
        f.write("/* Auto-generated by objz_gen_pools.py — do not edit */\n")
...
def main():
    ...
    if pools:
        for cls, (bsz, cnt, align) in sorted(pools.items()):
            print(f"objz_gen_pools: {cls} block_size={bsz} count={cnt} "
                  f"align={align}", file=sys.stderr)
    else:
        print("objz_gen_pools: no user classes with alloc calls found",
              file=sys.stderr)
